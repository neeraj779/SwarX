import { createDownloadLinks, createImageLinks } from '@/shared/utils/link.util';
...
export const songResponseDto = (
	song: z.infer<typeof songAPIResponseSchema>,
	...
	artists: {
		primary: song.more_info?.artistMap?.primary_artists?.map(artistMapResponseDto),
		featured: song.more_info?.artistMap?.featured_artists?.map(artistMapResponseDto),
		all: song.more_info?.artistMap?.artists?.map(artistMapResponseDto),
	},
	image: createImageLinks(song.image),
	downloadUrl: createDownloadLinks(song.more_info?.encrypted_media_url),
});

export const songLyricsResponseDto = (
